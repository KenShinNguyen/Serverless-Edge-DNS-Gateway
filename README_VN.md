...
*   **Dọn list tồn dư**: `cf_list_create` chỉ cập nhật đúng số chunk mà list hiện tại cần, không xoá chunk thừa từ lần chạy trước; mà rule lại được dựng từ *tất cả* list tên `CGPS List`, nên chunk cũ vẫn đang chặn bằng nội dung cũ (ăn quota 300k và nằm ngoài tầm kiểm soát của allowlist dùng chung). Khi chạy workflow thủ công, tick ô **`cleanup_stale_lists`** để xoá sạch rule + list CGPS rồi dựng lại từ đầu (lớp 2 ngưng lọc khoảng 1–2 phút trong lúc chạy).
