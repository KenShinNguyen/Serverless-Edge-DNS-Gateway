...
*   [rules/allowlists.txt](rules/allowlists.txt) is the **shared allowlist**: the Edge layer reads it directly and the Layer 2 workflow pulls the same file via raw URL, so both layers exempt the same domains. It is regenerated by the Layer 1 workflow, so permanent entries belong in that workflow's allowlist sources, not in the file itself. Because Layer 1 commits with `GITHUB_TOKEN` (which by design cannot trigger other workflows), Layer 2 picks up allowlist changes on its next daily run rather than immediately.
*   **Stale list cleanup**: `cf_list_create` only updates as many chunks as the current list needs and never deletes leftover chunks from an earlier run, while the rule is built from *every* list named `CGPS List` — so old chunks keep blocking their old contents, consuming the 300k quota and staying outside the shared allowlist's reach. When running the workflow manually, tick **`cleanup_stale_lists`** to delete the CGPS rule and lists and rebuild from scratch (Layer 2 stops filtering for the 1–2 minutes the run takes).
